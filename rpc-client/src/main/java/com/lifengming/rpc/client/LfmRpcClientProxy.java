...
import com.lifengming.rpc.remoting.netty.ChannelManager;
import com.lifengming.rpc.remoting.netty.ResponseFutureManager;
import com.lifengming.rpc.remoting.netty.RpcResponseFuture;
...
public class LfmRpcClientProxy {
    ...
    public Object clientProxy(final Class<?> interfaceClass) {
        return Proxy.newProxyInstance(interfaceClass.getClassLoader(), new Class<?>[]{interfaceClass}, (proxy, method, args) -> {
            ...
            if (null == channel) {
                throw new RpcException("Can't get channel for address" + serviceAddress);
            }
